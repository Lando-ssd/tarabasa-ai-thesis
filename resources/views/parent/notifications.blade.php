...
<style>
  ...
  *{box-sizing:border-box;} html,body{margin:0;padding:0;}
  ...
  h1{ font-family:'Baloo 2',sans-serif; font-size:26px; font-weight:700; margin:0; }

  .sub-row{ display:flex; align-items:center; justify-content:space-between; margin:6px 0 18px; flex-wrap:wrap; gap:10px; }
  .sub{ color:var(--slate-600); font-size:14.5px; font-weight:500; margin:0; }
  ...
  .filters{ display:flex; gap:9px; margin-bottom:24px; flex-wrap:wrap; }
  .filter-chip{
  ...
  .learner-section{ margin-bottom:30px; }
  .learner-header{ display:flex; align-items:center; gap:10px; margin-bottom:14px; }
  .learner-avatar{
    width:34px;height:34px;border-radius:50%; font-size:17px; overflow:hidden;
  ...
  .learner-name{ font-family:'Baloo 2',sans-serif; font-size:17px; font-weight:700; }

  .day-label{ font-size:11.5px; font-weight:800; letter-spacing:.06em; text-transform:uppercase; color:var(--slate-400); margin:16px 0 10px; }
  .day-label:first-of-type{ margin-top:2px; }
  ...
  .notif-icon.attention{ background:linear-gradient(155deg,#ff8a8a,var(--danger)); }
  .notif-icon.summary{ background:linear-gradient(155deg,#3fd4b0,var(--parent-teal)); }
...
<div class="shell">
  ...
  <h1>Notifications</h1>
  <div class="sub-row">
    <p class="sub">Updates about your children's reading, grouped by child.</p>
    @if ($unreadCount > 0)
  ...
  <div class="filters">
    <a href="{{ route('parent.notifications.index', ['filter' => 'all']) }}" class="filter-chip {{ $filter === 'all' ? 'active' : '' }}">All</a>
    <a href="{{ route('parent.notifications.index', ['filter' => 'attention']) }}" class="filter-chip {{ $filter === 'attention' ? 'active' : '' }}">Needs Attention</a>
    <a href="{{ route('parent.notifications.index', ['filter' => 'routine']) }}" class="filter-chip {{ $filter === 'routine' ? 'active' : '' }}">Updates</a>
  </div>

  @if ($byLearner->isEmpty())
    <div class="empty-note">No notifications yet — updates about your children's reading will show up here.</div>
  @else
    @foreach ($byLearner as $learnerName => $dayGroups)
      ...
      <div class="learner-section">
        <div class="learner-header">
          <span class="learner-avatar">
            @if ($anyNotification?->learner?->avatar_photo_path)
              <img src="{{ Storage::url($anyNotification->learner->avatar_photo_path) }}" alt="{{ $learnerName }}">
            @else
              {{ $anyNotification?->learner?->avatar_id ?? '🧒' }}
            @endif
          </span>
          <span class="learner-name">{{ $learnerName }}</span>
        </div>
                ...
                <form method="POST" action="{{ route('notifications.read', $notification) }}">
                  ...
                  <button type="submit" class="notif-item unread {{ $isAttention ? 'flagged' : '' }}">
                    ...
                    <span class="notif-body">
                      <span class="title">{{ $isAttention ? 'Needs a look' : $notification->type }}</span>
